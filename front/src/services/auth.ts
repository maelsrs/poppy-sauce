...
const translateDetail = (text: string): string => {
  ...
  if (
    lower.includes("ensure this value has at least") ||
    lower.includes("champ trop court") ||
...
const parseError = async (response: Response): Promise<Error> => {
  try {
    ...
    if (Array.isArray(detail) && detail.length > 0) {
      ...
      const msg = translateDetail(
        first?.msg || first?.message || "Erreur de validation",
      );
    ...
    if (response.status === 409)
      return new Error("Cette adresse email ou ce pseudo est déjà utilisé.");
    if (response.status === 422)
      return new Error("Données invalides. Merci de vérifier les champs.");
